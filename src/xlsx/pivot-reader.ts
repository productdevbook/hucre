...
export function parsePivotCacheDefinition(xml: string): PivotCache | undefined {
  ...
  const def = root.local === "pivotCacheDefinition" ? root : findChild(root, "pivotCacheDefinition")
...
export function parsePivotTable(xml: string): PivotTable | undefined {
  const root = parseXml(xml)
  const def = root.local === "pivotTableDefinition" ? root : findChild(root, "pivotTableDefinition")
  if (!def) return undefined
  ...
  const dataFieldOverrides = new Map<number, { name?: string; subtotal?: PivotDataFieldFunction }>()
  if (dataFields) {
